...
const moveDirectories = async (userInput) => {
  try {
    ...
    // Move old directories to the example directory or delete them
...
rl.question(
  "This will modify or delete the active project directories. Type RESET to continue: ",
  (confirmation) => {
    ...
    rl.question(
      "Move existing files to /example instead of deleting them? (Y/n): ",
      (answer) => {
        const userInput = answer.trim().toLowerCase() || "n";
        if (userInput === "y" || userInput === "n") {
          moveDirectories(userInput).finally(() => rl.close());
        } else {
          console.log("Invalid input. Reset cancelled; no files were changed.");
          rl.close();
        }
      },
    );
